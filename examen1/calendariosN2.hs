...
module CalendariosN2(printCalendario) where
...
meses2 ::  Year -> [(String, Year, Int, Int)]
meses2 y = [(head (drop i nombresmeses), y, (head (drop i (pdias y))), (head (drop i (longmeses y)))) | i <- [0..11]]

meses ::  Year -> [(String, Year, Int, Int)]
meses y = mesesaux nombresmeses y (pdias y) (longmeses y) 

mesesaux :: [String] -> Int -> [Int] -> [Int] -> [(String, Year, Int, Int)]
mesesaux [] y [] [] = []
mesesaux (nom:nomxs) y (pd:pdxs) (long:longxs) = (nom, y, pd, long) : mesesaux nomxs y pdxs longxs
...
fechas pd lm = fechasaux pd lm 1
...
fechasaux primdia long total 
 | total >= 42 = [["   "]]
 | total < primdia || total-primdia+1 > long  = ["   "] : fechasaux primdia long (total + 1)
 | otherwise = 
   if (total-primdia+1) < 10 then
    ["  " ++ show (total-primdia+1)] : fechasaux primdia long (total+1)
   else
    [" " ++ show (total-primdia+1)] : fechasaux primdia long (total+1)
